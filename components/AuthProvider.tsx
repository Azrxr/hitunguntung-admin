    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser && currentUser.email) {
        try {
          ...
          if (adminSnap.exists()) {
            // Update admin doc with their latest Google Data
            await setDoc(
              adminRef,
              {
                uid: currentUser.uid,
  ...
  if (isLoading && !isPublicRoute) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center">
        ...
        <p className="text-foreground animate-pulse">Memuat HitungUntung Admin...</p>
      </div>
    );
  }
  ...
  if ((!user || !isAdmin) && !isPublicRoute) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gold-500/20 rounded-full blur-[100px] -z-10" />
        ...
        <div className="glass-panel w-full max-w-sm p-8 rounded-2xl shadow-2xl flex flex-col items-center relative z-10 transition-all border border-card-border">
          ...
          <p className="text-center text-sm text-foreground/80 mb-6 font-medium">
            Masuk untuk mengakses panel admin HitungUntung
          </p>

          {errorMsg && (
            <div className="w-full bg-red-500/10 border border-red-500/50 text-red-400 p-4 rounded-xl text-sm text-center mb-6 shadow-[0_0_10px_rgba(239,68,68,0.2)]">
              {errorMsg}
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-[#fcfcfc] text-black font-extrabold rounded-xl flex items-center justify-center gap-3 hover:bg-gold-50 transition-colors shadow-lg active:scale-95"
          >
